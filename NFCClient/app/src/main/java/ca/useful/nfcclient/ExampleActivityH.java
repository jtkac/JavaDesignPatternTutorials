...
import ca.useful.nfcclient.databinding.ActivityExampleHBinding;
import cache.CacheExampleH;
import connector.ConnectorExampleH;
import service.ServiceExampleH;

/**
 * Design pattern first goes through lifecycle until context is populated and plugs in a service layer.
 *
 * The service layer instantiates a cache and calls the show(BaseCache c) method
 *
 * we then attach the cache to the transport language XML
 *
 * Next check for permission for a Base Directory using the requestDocumentTreePermission which uses semi custom URIs to persist context (no different than registering services in older framworks)
 * On permission result we persist the document tree intent in the content resolver
 * Now store the uri and treat that as the base for DocumentFile fromTreeUri method
 * Document structure is now accessible from the base directory chosen
 *
 * Document File also restricts base directory to at least one sub directory. We cannot choose system annotated directories either
 */
public class ExampleActivityH extends BaseActivity implements ConnectorExampleH {
    ...
    public void show(CacheExampleH cache) {
        ...
        if (!uri.isEmpty()) {
            ...
            List<DocumentFile> subFiles = Arrays.asList(accessibleDirectory.listFiles());
            cache.setDocumentFileRecyclerAdapter(new DocumentFileRecyclerAdapter(subFiles));
        } else {
